...
def test_everything_fits_under_the_real_budget():
    ...
    blocks = context.build_blocks(UIState(), [], [], _history(4, content_len=50), [], "what does this paper claim?", "skills: none")
...
def test_band_1_never_evicts_even_under_an_impossible_budget():
    ...
    blocks = context.build_blocks(UIState(), ["evidence"], [], _history(30), [Ref(kind="paper", id="p1", title="t")], "q", "skills: none")
...
def test_history_evicts_before_working_set():
    ...
    blocks = context.build_blocks(UIState(), [], [], _history(2, content_len=20), working_set_refs, "q", "skills: none")
    # Budget large enough for band 1 (now including the always-present skill index, H8) + the
    # working-set block, too small to also keep either history turn.
    messages, totals = context.assemble(blocks, budget=300)
...
def test_history_evicts_oldest_first():
    ...
    blocks = context.build_blocks(UIState(), [], [], _history(6, content_len=500), [], "q", "skills: none")
