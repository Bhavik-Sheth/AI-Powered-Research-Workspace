...
| `providers` | JSONB | NOT NULL, DEFAULT `'{}'` | `{provider: {ciphertext, nonce, last4, base_url?, validated_at, model_budgets?}}`. **Keys are AES-256-GCM ciphertext only; the master key lives in the OS keyring (`libsecret`), never here.** Local providers (Ollama, vLLM) store a `base_url` and **no key at all** — the UI must not demand one. The UI renders `…last4`. `model_budgets` is an optional `{model_string: token_budget}` map — keyed by the full stored model string (e.g. `"groq/llama-3.1-8b-instant"`, exactly the form `primary_model`/`auxiliary_model` store) rather than one flat per-provider ceiling, because a single provider can host several models with different TPM limits. Read by LLM Gateway's `_resolve_tier`; a model absent from the map leaves its requests unbounded. Learned automatically the first time a request overflows a provider's real limit — LLM Gateway parses the ceiling out of the provider's 429 body and persists it here before retrying (Bug Fix Plan Phase 2.1, superseding the single `request_token_budget` field from Phase 1.1). |
